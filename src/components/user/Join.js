...
import './Join.scss';
// import addImg from '../../assets/img/image-add.png';
...
const Join = () => {
  ...
  const fetchSignUpPost = async () => {
    /*
      기존 회원가입은 단순히 텍스트 객체로 모은 후 JSON으로 변환해서 요청 보내주면 끝.
      ...
      Blob은 이미지, 사운드, 비디오 같은 멀티미디어 파일을 바이트 단위로 쪼개어
      파일 손상을 방지하게 해주는 타입. -> multipart/form-data에도 허용됨.
    */
  ...
  const showThumnailHandelr = (e) => {
    // 첨부된 파일의 정보를 얻어오자
    const file = $fileTag.current.files[0];
    // const file = e.target.files[0]; // 위 코드와 동일
    ...
    if (
      ...
      // 맞지 않는 파일을 첨부한 것을 발견했다면, input 상태도 원래대로 돌려놓아야 한다.
      // 그렇지 않으면 잘못된 파일을 input 태그가 여전히 가지고 있게 됨. -> 서버 요청 시 에러 유발!
      $fileTag.current.value = '';
      // console.log('file: ', $fileTag.current.files[0]); // 텍스트 파일이 그대로 들어감
      return;
    }
            ...
            >
              ...
              >
                ...
                >
                  <img
                    // src={addImg}
                    // null이 아니라면 imgFile을 보여주고 null이라면 우항을 검사해서 기본 이미지 보여줌
